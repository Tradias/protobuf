...
class PROTOBUF_EXPORT UntypedMapBase {
 ...
 public:
  ...
  T* GetValue(NodeBase* node) const {
    ...
    ABSL_DCHECK(std::is_void_v<T> || VisitTypeInfo([](auto, auto value_type) {
                  return std::is_same_v<T, typename decltype(value_type)::type>;
                }));
...
template <typename F>
auto UntypedMapBase::VisitTypeInfo(F f) const {
  // Pass std::enable_if<true> as a standin for std::type_identity
  auto visit_key = [this, f](auto value_type) {
    switch (type_info_.key_type) {
      case TypeKind::kBool:
        return f(std::enable_if<true, bool>{}, value_type);
      case TypeKind::kU32:
        return f(std::enable_if<true, uint32_t>{}, value_type);
      case TypeKind::kU64:
        return f(std::enable_if<true, uint64_t>{}, value_type);
      case TypeKind::kString:
        return f(std::enable_if<true, std::string>{}, value_type);
      default:
        Unreachable();
    }
  };
  ...
  switch (type_info_.value_type) {
    case TypeKind::kBool:
      return visit_key(std::enable_if<true, bool>{});
    case TypeKind::kU32:
      return visit_key(std::enable_if<true, uint32_t>{});
    case TypeKind::kU64:
      return visit_key(std::enable_if<true, uint64_t>{});
    case TypeKind::kFloat:
      return visit_key(std::enable_if<true, float>{});
    case TypeKind::kDouble:
      return visit_key(std::enable_if<true, double>{});
    case TypeKind::kString:
      return visit_key(std::enable_if<true, std::string>{});
    case TypeKind::kMessage:
      return visit_key(std::enable_if<true, MessageLite>{});
    default:
      Unreachable();
  }
}
...
void UntypedMapBase::VisitAllNodes(F f) const {
  return VisitTypeInfo([&](auto key_type, auto value_type) {
    for (auto it = begin(); !it.Equals(EndIterator()); it.PlusPlus()) {
      f(static_cast<typename decltype(key_type)::type*>(it.node_->GetVoidKey()),
...
auto UntypedMapBase::VisitKey(NodeBase* node, F f) const {
  return VisitTypeInfo([&](auto key_type, auto) {
    return f(
        static_cast<typename decltype(key_type)::type*>(node->GetVoidKey()));
  });
}

template <typename F>
auto UntypedMapBase::VisitValue(NodeBase* node, F f) const {
  return VisitTypeInfo([&](auto, auto value_type) {
